...
def CorrectorFile(fid):
    NameFile=fid #nom complet amb path i tot(ho inahibilito per fer proves
##    print(NameFile)
    FileCorre=NameFile[:-4]+'-corrected.raw' #creo un nou arxiu
...
def Promig(vector):
    ...
    for i in range(31):
        ...
        for j in range(rep):
            ...
            H.append(h2) #obtinc el vectors dels 1ers, 2ons de cada repeticio
...
def group(a,indexcentral,Nnan,d):
    ...
    if indexcentral<=80 or indexcentral>=112:
        ...
        cont=0
        incr1=0#starts at 0
    ##    print('lon array',len(a),'index',index)
        while cond:
...
def Process(matrix,he,temps,D):#es el nucli de processat de la senyal
    ...
    for m in range(len(etaV)): #agafem cad altura per corregir els valors i determinen la ze per cada altura
        ...
        zewater.append(10**18*lamb**4*np.nansum(etaV[m])*Deltav/K2w)#calculat per Rayleight
        nde=[];vt=[];velHail=[]
    ...
    if np.sum(np.nanmean([vwaterR,vsnowR],axis=0))!=0:
            ####            BUCLE PER PINTAR LA SENYAL ORIGINAL, EL DEALIASING I LA QUE ENS QUEDEM
        colors = cm.rainbow(np.linspace(0, 1, len(etan)))
        fig=plt.figure()
        limitValueDeal=4#VALOR MOLT IMPORTANT
        DealMatrix=[]
        
        for o in range(len(etan)): #ara miren cada altura ja que tenim senyal en alguna
    ...
    ##            BUCLE PER DETERMINAR ESTAT ES POT FICAR COM UNA FUNCIO EXTERNA
            Indvel=speeddeal*(etaN_da/etaN_da)
                ##            selection the main figure
            ...
            if np.isnan(ReVect).all():
                ...
                if INewV[np.nanargmax(ReVect)]<0:#cas de neu ja que tenoim velocitat negativa
                    S=.5
                    L=5.
##                print('ws',round(vsnowR[o],2),'ww',round(vwaterMie[o],2),'w',round(INewV[np.nanargmax(ReVect)],2),'h',round(he[o],1))
                else:
            ...
            if np.isnan(S) and ~np.isnan(L):#case liquid, but possible wrong election
                state.append(10)
                if abs(L)>=limitValueDeal:#aquest es un possible cas d'error, on tenim que una mla eleccio del vector
                    ...
                    ReVect=np.concatenate((nc1,n1))#vector corregit
                    mov.append(1)
                else:
                    mov.append(np.nan)
            ...
            if ~np.isnan(S) and np.isnan(L):#case not liquid, but possible wrong election
                ...
                if S>=limitValueDeal:#aquest es un possible cas d'error, on tenim que una mala eleccio del vector
                    ...
                    nc1=np.concatenate((n1,n2))
                    ReVect=np.concatenate((nc1,n3))#vector corregit
                    mov.append(-1)
                else:
                    mov.append(np.nan)
            ...
            if abs(L)>(2*abs(sigma3)) and abs(S)>(2*abs(sigma3)):#cas complicat

                if abs(L)>=limitValueDeal and abs(S)>=limitValueDeal:#wrong election
                    if L-limitValueDeal<=0 or S-limitValueDeal<=0:#v esperada mes gran que la trobada agafem el vector de 128-32 a 128+32
                    ...
                    if L-limitValueDeal>0 or S-limitValueDeal>0:#v esperad es menor que la trobada agafem el vector de 64-32 a 64+32
        ...
        valuemax=[]
        for m in range(len(NewM)): #entrem en cada altura
    ...
    ##            print(len(iden))
        ...
        while len(indole)!=0:
            ...
            Vector=DealMatrix[int(indole[0]+1)]#index que esta malament

            indx=np.nanargmax(NewM[int(indole[0]+1)])
##            print('index del maxim', indx)
                
            Indvel=speeddeal*(Vector/Vector)
                
            if Dife[int(indole[0])]>0:#indica que hem agafat una vel superior
                if indx>(64+32):
                    IDX=indx-64
                else:
                    IDX=indx
            ...
            else:#indica que hem agafat una vel inferior
                if indx<(64+32):
                    IDX=indx+32
                else:
                    IDX=indx+64
...
##            print('diferenc',Dife[int(indole[0])])
...
##                if abs(sigma2)>=5:
##                    state[int(indole[0]+1)]=0.
##                    NewM[int(indole[0]+1)]=np.ones(len(CoVector))*np.nan
##                    print('too width')
                
##            print('noves L, S ',round(L,2),round(S,2),'spectral width',round(sigma2,2),comment,'w',round(w2,2))
            valuemax=[]
            for m in range(len(NewM)): #entrem en cada altura
    ...
    ##                print(NewM[m])
        ...
        ##            print(len(iden))
            ...
            indole=np.argwhere(abs(np.diff(valuemax))>8.)
##                fi=int(indole[0])
##            print('nou index dolent',indole)
##                if fi-ini==0:#estem al mateix index
##                    indole=indole[1:]
            Dife=np.diff(valuemax)
            if len(indole)!=0:
##                print('index inetressant',int(indole[0]))
        ...
        PIA=[]
        PIA.append(1)
        for m in range(len(NewM)):#entrem en un altura
            vector=NewM[m]
            ...
            if w==0.:
                w=np.nan
##            if 10*np.log10(ValueZe)>=50.:#hail case 
##                state[m]=-20
            ...
            if state[m]==10:#cas aigua liquida
                ...
                nde=[]
                indexFinded=[]
                for n in range(len(D[m])):#mirem cada valor en una altura
                ...
                #trobo el valor de diametre del valor mig de la vel
                diaWork=D[m]
                if w<=0 or w>=11:
                    if w<=0:#cas snow
                        diamHail=1
                ...
                if diamHail>=5:#np.log10(np.nansum(LastN))>=6 and w>=7.:# hail case
...
##            if state[m]==-20:#cas hail, de moment emprem la Ze SHA DE BUSCAR MES INFO
##                Mhail.append(NewM[m])
...
##                Mhail.append(NewM[m]*np.nan)
            ...
            if state[m]==20:#cas unknown, de moment emprem la Ze SHA DE BUSCAR MES INFO
                Munk.append(NewM[m])
                ##pendent trobar relacio hail vers Z
    ...
    return state,NewM,Z_da,lwc,rr,SnowRate,W,Sig,Sk,Noise,N_da,Nde,ZE,mov,VerTur,Snr,Kurt
...
def FindRealPeaks(matrix):#function to detcte real peaks, wherein a peak has a minuim 3 consecutives values
    Matrix=[]
...
def ScatExt(diameter,longW):#for 1 height gate
    ...
    for i in range(len(diameter)):
        ...
        if np.isnan(r):
            ...
            x = 2*np.pi*r/ag_lam;#te de ser adminesional, o sigui que ag_lam i r en les mateixes unitats
    ...
    for j in range(len(Pot)):
        ...
        if (meanv**2/(varv))>58 or varv==0:#bad signal
            ...
            for m in range(len(indMax)):
                Vmax=v[indMax[m]]
                VMAX.append(Vmax)#aquests son els valors del maxims que hem trobat

            maxim=np.argmax(VMAX)
        
            
            v[v>v[indMax[maxim]]]=np.nan
            PotWithOutHS.append(v)#senyal amb soroll, pero sensepics aberrants
        ...
        v2[v2<=1.2*soroll]=np.nan#fem que el senyal sigui superior a 2(ara 1.2) vegades el nivell de soroll
...
def PIAfunction(matrix,Diameters,sigmaext,r,da):
    LN=len(matrix)
    Ncorreg=[]
    PIA=[]
    PIA.append(1)
    ...
    for i in range(len(matrix)):#in one heigh gate
        Dnn=[]
        Diam=Diameters[i]
        Ext=sigmaext[i]
        Np=np.multiply(matrix[i],PIA[-1])

        dif=[]
        if da==1:
            for m in range(len(Diam)):#creo el vector de diferencies
                if m==0 or m==len(Diam)-1:
                    if m==0:
                        dif.append(Diam[m+1]-Diam[m])
                    if m==len(Diam)-1:
                        dif.append(abs(Diam[m-1]-Diam[m]))
                else:
                    if m<len(Diam)/2.:
                        dif.append(abs((Diam[m+1]-Diam[m-1])/2.))
                    else:
                        dif.append(abs((Diam[m-1]-Diam[m+1])/2.))
        else:
            ...
            for m in range(len(Diam)):#creo el vector de diferencies
                if m==0 or m==len(Diam)-1:
                    if m==0:
                        dif.append(Diam[m+1]-Diam[m])
                    if m==len(Diam)-1:
                        dif.append(Diam[m]-Diam[m-1])
                else:
                    dif.append((Diam[m+1]-Diam[m-1])/2.)

        Pro=[]

        for k in range(len(Np)):
            pro=Ext[k]*Np[k]*dif[k]
...
for name in dircf:
    ...
    NameFile=name #nom complet amb path i tot(ho inahibilito per fer proves)
    ...
    while True:
        ...
        if line=='':
            break
        NumberLines+=1
    NumberSpectre=NumberLines/67.#dividim per 67 per ser el nombre de linieas per espectre


    #if NumberSpectre!=(24.*60.*60./10.):
        #print('hi ha mes spectres del esperats en 1 dia')
    #else:
        #print('espectres esperats = als trobats')
    ...
    TimeInterval=60#seconds fo average the signal

    Timecount=0
    TimeCounter=0#servei per contar el minuts
    Cont=0
    #INICIEM LECTURA DE DADES
    ...
    while True:
        ...
        if line=='':
            if len(PotCorrSum)!=0:
                ...
                timeVec=timeList[0]+(60*TimeCounter)
                ...
                estat,NewMatrix,z_da,lwc,rr,SnowRate,w,sig,sk,Noi,DSD,NdE,Ze,Mov,velTur,snr,kur=Process(proeta,Hcolum[1:],timeVec,D)
        ...
        timeList.append(dat-int(unix2date(dat).second))#ho fiquem en el temps a cero segons
        TypeDate=columns[2] 
        DVS=columns[4] 
        DSN=columns[6] #numero de serie
        BW=columns[8] #Ample de banda
        CC=int(columns[10]) #constant de calibratge
        MDQ=columns[12:15] 
        ...
        #Read the heigh parameters (second line)
        H=f.readline()
        H=H.strip()
        Hcolum=H.split()
        ...
        #
        #Read the tranference function (third line)
        FT=f.readline()
        FT=FT.strip()
        FTcolum=FT.split()
        ...
        #constant value to conevrt F to f
        ...
        Pot=[]
        #convertim la potencia a una estructura mes convenient i ho dividem 
        #per la funcio tranferencia. Ara tenim contes f(n,i), amb tamnay (31,64)
        ...
        if (dat-timeVec)>=TimeInterval:
            ...
            ncShape2D = ('time','Height',)#per muntar les altures
            ...
            if Timecount==0:
                ...
                nc_state.description='Indicate the type from hydrometeor as liquid water (10), snow(-10), mixed (0), hail (-20) and unknown(20)'
                nc_state.units=''
